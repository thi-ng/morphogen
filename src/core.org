...
#+BEGIN_SRC clojure :noweb-ref tree-walk
  ...
  (defn walk-with-path*
    [node tree max-depth acc path]
    ;;(prn :d (tree-depth node) (:points node) tree)
    (if (< (tree-depth node) max-depth)
      (let [children (operator node tree)]
        (if children
          (loop [acc acc, children children, out (get tree kw-out), i 0]
            (if (and out children)
              (let [ctree (first out)]
                (recur
                 (if ctree
                   (walk-with-path* (first children) ctree max-depth acc (conj path i))
                   acc)
                 (next children) (next out) (inc i)))
              acc))
          (conj! acc [path (g/faces node)])))
      (conj! acc [path (g/faces node)])))
  
  (defn walk-with-path
    ([seed tree] (walk-with-path seed tree 1e6))
    ([seed tree max-depth]
       (persistent! (walk-with-path* seed tree max-depth (transient []) []))))
#+END_SRC
