...
#+BEGIN_SRC clojure :noweb-ref operators
  (defmethod operator [BoxNode :replicate]
    [{[a b c d e f g h] :points :as node} {{:keys [dir] :or {dir :n}} :args}]
    [node
     (BoxNode.
      (case dir
        :f (let [ba (g/- b a)
...
#+BEGIN_SRC clojure :noweb-ref op-ctors
  ...
  (defn subdiv
    ...
    {:op :sd
     :args (if num
             {:cols num :rows num :slices num}
             {:cols cols :rows rows :slices slices})
...
#+BEGIN_SRC clojure :noweb-ref meshing
  ...
  (defn generate-mesh
    ([seed tree] (generate-mesh seed tree 1e6))
    ([seed tree max-depth] (union-mesh (walk seed tree max-depth))))
