...
#+BEGIN_SRC clojure :noweb-ref operators
  (defprotocol POperator
    (operator-args [_] "Returns map of arguments supported/required by this op")
    (apply-operator [_ node] "Applies op to node, returns seq of child nodes"))
#+END_SRC
...
#+BEGIN_SRC clojure :noweb-ref operators
  ...
  (defn subdivide-inset
    [[a b c d e f g h :as points]
     {i :inset dir :dir :or {i 0.1 dir :y}}]
    (let [inset #(q/inset-quad % i)]
      (case dir
        :x (let [[a2 b2 f2 e2] (inset [a b f e])
                 [c2 d2 h2 g2] (inset [c d h g])]
             [[b c d a b2 c2 d2 a2]
              [f2 g2 h2 e2 f g h e]
              [b c c2 b2 f g g2 f2]
              [a2 d2 d a e2 h2 h e]
              [b2 c2 d2 a2 f2 g2 h2 e2]])
        :y (let [[a2 b2 c2 d2] (inset [a b c d])
                 [e2 f2 g2 h2] (inset [e f g h])]
             [[b2 b c c2 f2 f g g2]
              [a a2 d2 d e e2 h2 h]
              [a b b2 a2 e f f2 e2]
              [d2 c2 c d h2 g2 g h]
              [a2 b2 c2 d2 e2 f2 g2 h2]])
        :z (let [[a2 d2 h2 e2] (inset [a d h e])
                 [b2 c2 g2 f2] (inset [b c g f])
...
#+BEGIN_SRC clojure :noweb-ref operators
  (defmethod operator [BoxNode :extrude]
    [node {{:keys [dir len] :or {dir :n len 1.0}} :args}]
  ...
  (defmethod operator [BoxNode :ext-prop]
    ...
    [(BoxNode.
      (case dir
        :e [a b (g/+ c (g/normalize (g/- c b) len)) (g/+ d (g/normalize (g/- d a) len))
            e f (g/+ g (g/normalize (g/- g f) len)) (g/+ h (g/normalize (g/- h e) len))]
        :w [(g/+ a (g/normalize (g/- a d) len)) (g/+ b (g/normalize (g/- b c) len)) c d
            (g/+ e (g/normalize (g/- e h) len)) (g/+ f (g/normalize (g/- f g) len)) g h]
        :f [a (g/+ b (g/normalize (g/- b a) len)) (g/+ c (g/normalize (g/- c d) len)) d
            e (g/+ f (g/normalize (g/- f e) len)) (g/+ g (g/normalize (g/- g h) len)) h]
        :b [(g/+ a (g/normalize (g/- a b) len)) b c (g/+ d (g/normalize (g/- d c) len))
            (g/+ e (g/normalize (g/- e f) len)) f g (g/+ h (g/normalize (g/- h g) len))]
        :n [a b c d
            (g/+ e (g/normalize (g/- e a) len)) (g/+ f (g/normalize (g/- f b) len))
            (g/+ g (g/normalize (g/- g c) len)) (g/+ h (g/normalize (g/- h d) len))]
        :s [(g/+ a (g/normalize (g/- a e) len)) (g/+ b (g/normalize (g/- b f) len))
            (g/+ c (g/normalize (g/- c g) len)) (g/+ d (g/normalize (g/- d h) len))
            e f g h])
      node (inc (tree-depth node)))])
#+END_SRC
...
#+BEGIN_SRC clojure :noweb-ref operators
  (defmethod operator [BoxNode :scale-edge]
    [{[a b c d e f g h] :points :as node}
     {{:keys [edge sym scale length] :or {scale 0.5}} :args}]
    (let [s (* scale 0.5)
...
#+BEGIN_SRC clojure :noweb-ref meshing
  (defn circle-lattice-seg
    ...
    (let [theta (/ m/PI n)
          off (vec3 0 0 h)
          points (g/vertices (g/rotate (g/as-polygon (c/circle) n) (- (- m/HALF_PI) theta)))
          [b c] (map vec3 points)
