...
via the =faces= method of the =INode= protocol:

**** INode protocol

The =INode= protocol is part of the abstraction mechanism to allow for
...
=IFaceAccess= protocols defined in the =thi.ng.geom.core= namespace to
...
Additionally, this node type also implements the =ISubdivide= protocol
...
least implement the =INode= protocol) in order to be transformed, the
...
Unlike the route taken for geometry nodes (using protocols &
defrecords), operator nodes (the entire tree, really) are simple Clojure
maps with this basic structure:
...
tree of each created object. The functions =operator-seq= and
