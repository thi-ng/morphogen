...
#+BEGIN_SRC clojure :noweb-ref op-ctors
  (defn subdiv
    [& {:keys [cols rows slices out] :or {cols 1 rows 1 slices 1}}]
    {:op :sd
     :args {:cols cols :rows rows :slices slices}
     :out (vec (or out (repeat (* cols rows slices) {})))})

  (defn subdiv-inset
    [& {:keys [dir inset out] :or {dir :y inset 0.25}}]
    {:op :sd-ins
     :args {:dir dir :inset inset}
     :out (vec (or out (vec (repeat 5 {}))))})

  (defn reflect
    [& {:keys [dir out] :or {dir :n}}]
    {:op :refl
     :args {:dir dir}
     :out (vec (or out [{} {}]))})
#+END_SRC

** AST walking & node processing
...
#+BEGIN_SRC clojure :noweb-ref meshing
  ...
  (defn save-mesh
    [seed tree max-depth]
    (with-open [o (io/output-stream "p.ply")]
