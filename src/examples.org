...
The following code generates the mesh shown in the image above: A
sphere segment is used a seed shape to create a number of hexagons,
which arrange themselves in a spherical constellation. The only
operator used here is =reflection=. Since the operator tree is context
free & distinct from any actual node hierarchy, we can encode various
