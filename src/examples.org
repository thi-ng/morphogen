...
#+BEGIN_SRC clojure :tangle ../babel/examples/ex03.clj :noweb yes :mkdirp yes :padline no
  ...
  (def t
    ...
    (let [ring (mg/apply-recursively (mg/reflect :dir :e) 5 [1] 1)
...
This structure very nicely demonstrates the potential of the
morphogenic approach and uses the same base elements as above, but
with the tree having much deeper nesting to create the additional
structures, which are unfolded & extruded from the 60 individual
hexagon line segments.

*** TODO simlify virus.clj
