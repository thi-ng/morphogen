...
Each examples defined below will be tangled into the
=/babel/examples/= subfolder and can be directly loaded into a running
...
which due to the chosen angles of the seed shape automatically arrange
themselves in a spherical constellation. The =reflection= operator is
the only one used here. Since the operator tree is context free &
distinct from any actual node hierarchy, we can encode various
sub-transformations using simple functions:

#+BEGIN_SRC clojure :tangle ../babel/examples/ex03.clj :noweb yes :mkdirp yes :padline no
  (ns ex03
    <<require-mg>>)
  
  (defn apply-recursive1
    "Recursively injects tree into itself n times. Each time original
    tree given is injected at index `id` of the `:out` child node vec"
    [tree n id]
  ...
  (def t
    ...
    (let [ring (apply-recursive1 (mg/reflect :dir :e) 5 1)
          refl-ring (mg/reflect :dir :n :out [{} ring])
          path #(interleave (repeat :out) %)
          inject #(-> ring
                      (assoc-in (path [1 1 0]) %)
                      (assoc-in (path [1 1 1 1 0]) %))
          splice (mg/reflect :dir :s :out [{} (inject refl-ring)])]
      (mg/reflect :dir :s :out [(inject splice) (inject refl-ring)])))
  
  (mg/save-mesh (mg/seed (mg/sphere-seg 6 0.25 0.0955 0.2)) t)
#+END_SRC
